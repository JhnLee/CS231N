...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    dW2 = np.dot(h.T, dscores) / N #(H,C)
    dW2 += reg*W2
    db2 = np.sum(dscores, axis = 0) / N

    dW1 = np.dot(X.T, dz) / N #(D,H)
    dW1 += reg*W1
  ...
  def predict(self, X):
    ...
    ###########################################################################
    z = np.dot(X,self.params['W1']) + self.params['b1'] #(N,H)
    h = np.maximum(0, z) #ReLU  
    scores = np.dot(h, self.params['W2']) + self.params['b2'] #(N,C)
